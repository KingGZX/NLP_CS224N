...
def naiveSoftmaxLossAndGradient(
    ...
    probability = softmax(outsideVectors.dot(centerWordVec))
    prob = probability[outsideWordIdx]
    loss = -np.log(prob)
    gradCenterVec = outsideVectors[outsideWordIdx] - outsideVectors.T.dot(probability)
    gradOutsideVecs = -probability.reshape(-1, 1).dot(centerWordVec.reshape(1, -1))
    gradOutsideVecs[outsideWordIdx] += centerWordVec
    # because loss function has a negtive sign!
    gradCenterVec, gradOutsideVecs = -gradCenterVec, -gradOutsideVecs
...
def negSamplingLossAndGradient(
    ...
    negtiveExamples = outsideVectors[negSampleWordIndices]               # (k, d)
    activate1 = sigmoid(centerWordVec.dot(outsideVectors[outsideWordIdx]))
    activate2 = sigmoid(-negtiveExamples.dot(centerWordVec))
    loss = -(np.log(activate1) + np.sum(np.log(activate2)))
    """
    这里要熟悉一点就是sigmoid求导就是 y * (1-y),其中y是sigmoid输出概率
    再配合log求导的话,y会直接消掉.所以就变得很简单了.
    
    对于负采样part,可以把求和式拆开来理解. 最后会发现就是负样本向量 * 自己的补概率(1 - y),所以就改写成矩阵形式就很简单了.
    """
    gradCenterVec = (1 - activate1) * outsideVectors[outsideWordIdx] - negtiveExamples.T.dot(1 - activate2)
    """ 
    Version 1. 没想到太好的主意,所以暴力对每个负样本循环
    """
    gradOutsideVecs = np.zeros_like(outsideVectors)
    for index, negIndex in enumerate(negSampleWordIndices):
        """ 
        呃呃呃,这里注意一下,卡了一下午。。。。。
        就是我一开始写的是activate2[negIndex],后来发现不是那么回事。。。。
        所以用了enumerate来获取index,才可以得到正确的答案..
        好麻烦,不想玩文字游戏,可以自行体会一下我previous错误写法...
        """
        gradOutsideVecs[negIndex] -= centerWordVec * (1 - activate2[index])
    gradOutsideVecs[outsideWordIdx] += centerWordVec * (1 - activate1)
    gradCenterVec, gradOutsideVecs = -gradCenterVec, -gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    """ 
    就是对自己上面写的东西的一个整合运用,不难
    
    无非这里要自己通过word2Ind来获取一下index,然后根据index来获取向量并且把梯度置于矩阵的合适位置
    """
    
    centerIdx = word2Ind[currentCenterWord]
    for outsideWord in outsideWords:
        outsideIdx = word2Ind[outsideWord]
        temploss, grad1, grad2 = word2vecLossAndGradient(centerWordVectors[centerIdx], outsideIdx, outsideVectors, dataset)
        gradCenterVecs[centerIdx] += grad1
        gradOutsideVectors += grad2
        loss += temploss
