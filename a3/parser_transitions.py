...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            # 从buffer取第一个词放到stack里
            myStr = self.buffer.pop(0)
            self.stack.append(myStr)
        elif transition == "LA":
            # 将stack次顶的词和最顶的词加入弧,并且移去次顶的词
            if len(self.stack) > 1:
                top1 = self.stack.pop()
                top2 = self.stack.pop()
                self.dependencies.append((top1, top2))
                self.stack.append(top1)
        elif transition == "RA":
            if len(self.stack) > 1:
                top1 = self.stack.pop()
                top2 = self.stack.pop()
                self.dependencies.append((top2, top1))
                self.stack.append(top2)
                
        """
        实际上对于 stack size的判断略微有点多余,主要是自己起初写bug的时候每次这儿都报错引起的,后来发现是
        minibatch parse写错引起的这里报错
        """
