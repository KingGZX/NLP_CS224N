...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.randn(self.embed_size * n_features, self.hidden_size))
    ...
    def embedding_lookup(self, w):
        ...
        # shape is (batch_size * n_fetures, self.embeddings.shape[1])
        embedding = torch.index_select(self.embeddings, 0, torch.flatten(w)) 
        x = embedding.view(w.shape[0], -1)
    ...
    def forward(self, w):
        ...
        """
        一开始矩阵相乘直接用的 * , shape出错了, * 执行的是element wise的乘法操作
        """
        out1 = torch.mm(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        out1 = F.relu(out1)                # init 时并没有生命 但在 TODO 里提到了ReLU层
        out2 = self.dropout(out1)
        logits = torch.mm(out2, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
